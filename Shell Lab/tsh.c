 ...
 *
 * PureWhiteVK / yl_xiao@outlook.com
 */
#include <ctype.h>
#include <errno.h>
#include <signal.h>
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    eval(cmdline);
    // should we fflush both stdout and stderr ?
    fflush(stdout);
    fflush(stdout);
  }
...
void eval(char *cmdline) {
  ...
  if (!builtin_cmd(argv)) {
    sigset_t mask, prev_all, mask_all;
    sigfillset(&mask_all);
    sigemptyset(&mask);
...
int builtin_cmd(char **argv) {
  if (!strcmp(argv[0], "&")) {
    ...
    // send sighup to all child process ?
    exit(0);
    return 1;
  }
...
void sigchld_handler(int sig) {
  ...
  if (verbose) {
    // using printf here may cause trouble!
    ...
    if (WIFEXITED(status)) {
      if (verbose) {
        ...
        printf("sigchld_handler: Job [%d] (%d) terminates OK (status %d)\n",
               j->jid, j->pid, WEXITSTATUS(status));
        fflush(stdout);
      }
      sigprocmask(SIG_BLOCK, &mask, &prev);
      deletejob(jobs, pid);
